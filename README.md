...
- 预览图片始终裁剪在当前中/宽磁贴边界内，缩放或移动时不会绘制到外层预览区域。
- 图片放大时可在裁剪范围内移动，缩小时也可在透明空白范围内自由调整左右和上下位置。
- 图片在 `1.0×` 恰好铺满磁贴时仍保留平移范围，左右和上下滑块不会失效。
...
- 新建磁贴由独立的无窗口桌面入口直接执行目标，点击时不会打开 CYT 配置界面；旧版磁贴需重新固定一次完成迁移。
